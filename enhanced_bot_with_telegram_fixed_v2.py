...
class EnhancedMegaTrendBotWithTelegram:
    ...
    def normalize_price(self, price: float, symbol_info) -> float:
        """Fiyatı sembolün digit sayısına göre normalize et"""
        if symbol_info is None:
            return round(price, 5)  # Varsayılan 5 digit
        
        digits = symbol_info.digits
        return round(price, digits)

    def validate_stops(self, symbol_info, order_type: int, price: float, stop_loss: float, take_profit: float) -> Tuple[float, float]:
        """SL ve TP değerlerini doğrula ve düzelt"""
        if symbol_info is None:
        ...
        if order_type == mt5.ORDER_TYPE_BUY:
            # BUY için SL price'ın altında, TP price'ın üstünde olmalı
            min_sl = price - stops_level
            min_tp = price + stops_level
            
            if stop_loss > min_sl:
                stop_loss = min_sl
                self.logger.warning(f"SL düzeltildi: {stop_loss}")
                
            if take_profit < min_tp:
                take_profit = min_tp
                self.logger.warning(f"TP düzeltildi: {take_profit}")
                
        else:  # SELL
            # SELL için SL price'ın üstünde, TP price'ın altında olmalı
        ...
        stop_loss = self.normalize_price(stop_loss, symbol_info)
        take_profit = self.normalize_price(take_profit, symbol_info)
    ...
    def calculate_stop_loss_take_profit(self, signal_data):
        ...
        # SL ve TP'yi doğrula ve düzelt
        order_type = mt5.ORDER_TYPE_BUY if signal in ["BUY", "STRONG_BUY"] else mt5.ORDER_TYPE_SELL
        stop_loss, take_profit = self.validate_stops(symbol_info, order_type, current_price, stop_loss, take_profit)
    ...
    def place_order(self, order_type, stop_loss, take_profit, lot_size=None):
        """Emir ver - Gelişmiş hata kontrolü ile"""
        symbol_info = mt5.symbol_info(self.symbol)
        if symbol_info is None:
        ...
        # Fiyatları normalize et
        price = self.normalize_price(price, symbol_info)
        stop_loss = self.normalize_price(stop_loss, symbol_info)
        take_profit = self.normalize_price(take_profit, symbol_info)
        
        # SL ve TP'yi tekrar doğrula
        stop_loss, take_profit = self.validate_stops(symbol_info, order_type, price, stop_loss, take_profit)
        
        # Debug log
        self.logger.info(f"Emir detayları: Price={price}, SL={stop_loss}, TP={take_profit}, Volume={volume}")
        ...
        if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            self.logger.error(f"Emir başarısız: {error_msg}")
            
            if self.telegram:
                self.telegram.send_error_notification("Emir Hatası", error_msg)
            return False
            ...
            try:
                ...
                if signal in ["BUY", "STRONG_BUY"] and buy_count < self.max_positions:
                    ...
                    if sl is None or tp is None:
                        self.logger.warning("SL veya TP hesaplanamadı")
                        time.sleep(5)
                        continue
