...
$clientname=$ts3->clientGetNameFromDbid($cldbid);
...
if (isset($_POST['editall']))
	...
	foreach($allpermsedit AS $key => $value)
		...
		$ts3->clientDeletePerm($cldbid, $delperms);
		
	}

$clpermlist=$ts3->clientPermList($cldbid);
...
<table class="border" cellpadding="0" cellspacing="0" style="width:100%;">
	<tr>
		<td class="thead" colspan="7" style="text-align:right">
		<form method="post" action="index.php?site=cleditperm&amp;cldbid=<?php echo $cldbid; ?>&amp;port=<?php echo $port; ?>">
	...
	<tr>
		<td class="thead" colspan="8"><?php echo "(".$cldbid.")".$clientname['0']['name']." ".$lang['permissionlist'];?></td>
	</tr>
	<tr>
		<td class="thead" style="width:51px">&nbsp;</td>
		<td class="thead" style="width:45px"><?php echo $lang['permid']; ?></td>
		<td class="thead" style="width:360px"><?php echo $lang['permname']; ?></td>
		<td class="thead" style="width:50px"><?php echo $lang['value']; ?></td>
		<td class="thead" style="width:50px"><?php echo $lang['skip']; ?></td>
		<td class="thead" style="width:100px"><?php echo $lang['options']; ?></td>
	...
	foreach($allperms AS $value)
		...
		if($value['permname']=='b_channel_info_view')
			{
			echo 		"<tr>",
							"<td class='subcat' style='width:50px'>&nbsp;</td>",
		...
		foreach($allperms AS $value2)
			...
			if(substr($value2['permname'], 22) == substr($value['permname'], 2))
				{
				$value['grant']=isset($value2['permvalue']) ? $value2['permvalue']:'';
				$value['grantpermid']=$value2['permid'];
				$value['grantav']=isset($value2['available']) ? $value2['available']:'';
				}
			}
		...
		?>
		<tr>
		<form method="POST" action="index.php?site=cleditperm&amp;port=<?php echo $port; ?>&amp;cldbid=<?php echo $cldbid; ?>" name="<?php echo $value['permid']; ?>">
			<td class="<?php echo $td_col;?>" style="width:50px">&nbsp;</td>
			<td class="<?php echo $td_col;?>" style="width:45px"><?php echo $value['permid']; ?></td>
			<td class="<?php echo $td_col;?>" style="width:360px"><?php echo $value['permname']; ?> <br />(<?php echo $value['permdesc']; ?>)</td>
			<td class="<?php echo $td_col;?>" style="width:50px">
			<?php
			if(substr($value['permname'], 0, 2)!='i_')
				{
				$check='';
				if(isset($value['permvalue']) AND $value['permvalue']==1)
					{
			...
			<?php
			$check='';
			if(isset($value['permskip']) AND $value['permskip']==1)
				{
				$check="checked='checked'";
				}
			?>
			...
			<?php
			if(isset($value['available']) AND $value['available']==1)
				{
