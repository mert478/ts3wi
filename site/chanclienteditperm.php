<form method="post" action="index.php?site=chanclienteditperm&amp;port=<?php echo $port; ?>">
	<table class="border" cellpadding="1" cellspacing="0">
		<tr>
			<td class="thead" colspan="2"><?php echo $lang['chosechanclient']; ?></td>
		</tr>
		<tr>
			<td class="green1"><?php echo $lang['channelid']; ?></td>
			<td class="green1"><input type="text" name="cid" value="" /></td>
		</tr>
		<tr>
			<td class="green2"><?php echo $lang['clientdbid']; ?></td>
			<td class="green2"><input type="text" name="cldbid" value="" /></td>
		</tr>
		<tr>
			<td class="green1"><?php echo $lang['option']; ?>:</td>
			<td class="green1"><input class="button" type="submit" name="showperms" value="<?php echo $lang['view']; ?>" /></td>
		</tr>
	</table>
</form>
<?php 
if(isset($_POST['cid']) AND isset($_POST['cldbid']))
	...
	$channelinfo=$ts3->channelInfo($_POST['cid']);
	$channelname=$channelinfo['channel_name'];
	$clientname=$ts3->clientGetNameFromDbid($_POST['cldbid']);
	...
	if (isset($_POST['editall']))
		...
		foreach($allpermsedit AS $key => $value)
			...
			$ts3->channelClientDeletePerm($_POST['cid'], $_POST['cldbid'], $delperms);
...
if($getlist!=false)
	...
	<tr>
		...
		<input type="hidden" name="cldbid" value="<?php echo $_POST['cldbid']; ?>" />
	...
	<tr>
		<td class="thead" colspan="6"><?php echo "(".$_POST['cid'].") ".$channelname." (".$_POST['cldbid'].") ".$clientname." ".$lang['permissionlist'];?></td>
	</tr>
	<tr>
	...
	foreach($allperms AS $value)
		...
		?>
		<tr>
		<form method="POST" action="index.php?site=chanclienteditperm&amp;port=<?php echo $port; ?>" name="<?php echo $value['permid']; ?>">
