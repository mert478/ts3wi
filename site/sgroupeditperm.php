...
if (isset($_POST['editall']))
	...
	foreach($allpermsedit AS $key => $value)
		...
		elseif(isset($value['available']) OR !isset($value['available']) AND !empty($value['value']) OR !isset($value['available']) AND !empty($value['skip']) OR !isset($value['available']) AND !empty($value['negated']))
			...
			(!isset($value['skip']) or empty($value['skip'])) ? $editperms[$key]['skip']='0' : $editperms[$key]['skip']=$value['skip'];
			(!isset($value['negated']) or empty($value['negated'])) ? $editperms[$key]['negated']='0' : $editperms[$key]['negated']=$value['negated'];
			}
		}
...
$sgrouplist=$ts3->serverGroupPermList($sgid);
...
<table  class="border" cellpadding="0" cellspacing="0" style="width:100%">
	<tr>
		<td class="thead" colspan="8" style="text-align:right">
		<form method="post" action="index.php?site=sgroupeditperm&amp;sgid=<?php echo $sgid; ?>&amp;port=<?php echo $port; ?>">
	...
	<tr>
		<td class="thead" colspan="8"><?php echo "(".$sgid.")".$sgroupname." ".$lang['permissionlist'];?></td>
	</tr>
	<tr>
		<td class="thead" style="width:55px">&nbsp;</td>
		<td class="thead" style="width:55px"><?php echo $lang['permid']; ?></td>
		<td class="thead" style="width:370px"><?php echo $lang['permname']; ?></td>
		<td class="thead" style="width:58px"><?php echo $lang['value']; ?></td>
		<td class="thead" style="width:58px"><?php echo $lang['skip']; ?></td>
		<td class="thead" style="width:58px"><?php echo $lang['negated']; ?></td>
		<td class="thead" style="width:98px"><?php echo $lang['options']; ?></td>
		<td class="thead" style="width:98px"><?php echo $lang['grant']; ?></td>
	</tr>
	<?php
	$change_col=1;
	...
	foreach($allperms AS $value)
			...
			echo "<tr>",
				 ...
				 "<tr>",
					 ...
					 "<table style=\"width:100%;border-collapse:collapse;\" cellpadding=\"0\" cellspacing=\"0\">",
						 "<tr>",
							 "<td class='subcat' style=\"width:58px\">&nbsp;</td>",
		...
		?>
		<tr>
		<form method="POST" action="index.php?site=sgroupeditperm&amp;port=<?php echo $port; ?>&amp;sgid=<?php echo $sgid; ?>">
			<td class="<?php echo $td_col;?>" style="width:55px">&nbsp;</td>
			<td class="<?php echo $td_col;?> center" style="width:55px"><?php echo $value['permid']; ?></td>
			<td class="<?php echo $td_col;?>" style="width:370px"><?php echo $value['permname']; ?> <br />(<?php echo $value['permdesc']; ?>)</td>
			<td class="<?php echo $td_col;?> center" style="width:58px">
			<?php
			...
			?>
			<input type='checkbox' name="perm[<?php echo $value['permid']; ?>][skip]" value="1" <?php echo $check; ?> />
			</td>
			<td class="<?php echo $td_col;?> center" style="width:58px">
			<?php
			$check='';
			if($value['permnegated']==1)
				{
				$check="checked='checked'";
				}
			?>
			<input type='checkbox' name="perm[<?php echo $value['permid']; ?>][negated]" value="1" <?php echo $check; ?> />
			</td>
			<td class="<?php echo $td_col;?>" style="width:98px">
			<?php
