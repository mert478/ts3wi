...
class EnhancedMegaTrendBot:
        ...
        for attempt in range(3):
            if mt5.initialize():
                ...
                if account_info is not None:
                    ...
                    # AutoTrading durumunu kontrol et
                    if not self.check_autotrading_enabled():
                        self.logger.error("AutoTrading devre dışı! Lütfen MT5'te AutoTrading'i etkinleştirin.")
                        self.logger.info("Çözüm: MT5 ana penceresinde 'AutoTrading' butonuna tıklayın veya Ctrl+E tuşlarına basın")
                        mt5.shutdown()
                        return False
    ...
    def get_error_description(self, error_code):
        ...
        error_descriptions = {
            10027: "TRADE_RETCODE_AUTOTRADING_DISABLED - AutoTrading devre dışı",
            10028: "TRADE_RETCODE_CLIENT_DISABLES_AT - İstemci AutoTrading'i devre dışı bıraktı",
            10006: "TRADE_RETCODE_REJECT - İstek reddedildi",
            10007: "TRADE_RETCODE_CANCEL - İstek iptal edildi",
            10008: "TRADE_RETCODE_PLACED - Emir yerleştirildi",
            10009: "TRADE_RETCODE_DONE - İstek tamamlandı",
            10010: "TRADE_RETCODE_DONE_PARTIAL - İstek kısmen tamamlandı",
            10011: "TRADE_RETCODE_ERROR - İstek işleme hatası",
            10012: "TRADE_RETCODE_TIMEOUT - İstek zaman aşımı",
            10013: "TRADE_RETCODE_INVALID - Geçersiz istek",
            10014: "TRADE_RETCODE_INVALID_VOLUME - Geçersiz hacim",
            10015: "TRADE_RETCODE_INVALID_PRICE - Geçersiz fiyat",
            10016: "TRADE_RETCODE_INVALID_STOPS - Geçersiz stop seviyeleri",
            10017: "TRADE_RETCODE_TRADE_DISABLED - Ticaret devre dışı",
            10018: "TRADE_RETCODE_MARKET_CLOSED - Piyasa kapalı",
            10019: "TRADE_RETCODE_NO_MONEY - Yetersiz para",
            10020: "TRADE_RETCODE_PRICE_CHANGED - Fiyat değişti",
            10021: "TRADE_RETCODE_PRICE_OFF - Fiyat kapalı",
            10022: "TRADE_RETCODE_INVALID_EXPIRATION - Geçersiz son kullanma",
            10023: "TRADE_RETCODE_ORDER_CHANGED - Emir değişti",
            10024: "TRADE_RETCODE_TOO_MANY_REQUESTS - Çok fazla istek",
            10025: "TRADE_RETCODE_NO_CHANGES - Değişiklik yok",
            10026: "TRADE_RETCODE_SERVER_DISABLES_AT - Sunucu AutoTrading'i devre dışı bıraktı",
            10029: "TRADE_RETCODE_ONLY_REAL - Sadece gerçek hesaplar",
            10030: "TRADE_RETCODE_LIMIT_ORDERS - Limit emirler",
            10031: "TRADE_RETCODE_LIMIT_VOLUME - Hacim limiti",
            10032: "TRADE_RETCODE_INVALID_ORDER - Geçersiz emir",
            10033: "TRADE_RETCODE_POSITION_CLOSED - Pozisyon kapatıldı"
        }
        return error_descriptions.get(error_code, f"Bilinmeyen hata kodu: {error_code}")
    ...
    def place_order(self, order_type, stop_loss, take_profit, lot_size=None):
        ...
        if not self.check_autotrading_enabled():
            self.logger.error("AutoTrading devre dışı - Emir verilemez")
            return False
        ...
        # Stop Loss ve Take Profit değerlerini symbol özelliklerine göre ayarla
        ...
        self.logger.info(f"Emir gönderiliyor: {request}")
        
        result = mt5.order_send(request)
        if result is None:
            self.logger.error("Emir sonucu None döndü")
            return False
        
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            error_desc = self.get_error_description(result.retcode)
            self.logger.error(f"Emir başarısız: {error_desc}")
            self.logger.error(f"Emir detayları: {result}")
            
            # Özel hata durumları için çözüm önerileri
            if result.retcode == 10027:  # AUTOTRADING_DISABLED
                self.logger.error("ÇÖZÜM: MT5'te AutoTrading butonuna tıklayın veya Ctrl+E tuşlarına basın")
            elif result.retcode == 10019:  # NO_MONEY
                self.logger.error("ÇÖZÜM: Hesabınızda yeterli bakiye yok")
            elif result.retcode == 10018:  # MARKET_CLOSED
                self.logger.error("ÇÖZÜM: Piyasa kapalı, ticaret saatlerini kontrol edin")
            
            return False
        
        self.logger.info(f"Emir başarılı: {'BUY' if order_type == mt5.ORDER_TYPE_BUY else 'SELL'} - {volume} lot")
        self.logger.info(f"Ticket: {result.order}, Price: {result.price}")
        return True
    ...
    def run_bot(self):
        ...
        if not self.initialize_mt5():
            ...
            input("MT5 bağlantı hatası! Çıkmak için Enter tuşuna basın...")
            return
            ...
            try:
                ...
                if cycle_count % 100 == 0:
                    ...
                    if account_info:
                        ...
                        # AutoTrading durumunu periyodik kontrol et
                        if not self.check_autotrading_enabled():
                            self.logger.warning("AutoTrading devre dışı tespit edildi - Bot beklemede")
                            time.sleep(60)
                            continue
